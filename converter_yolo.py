...
def get_annotations_dataframe(annotations_bbox, class_id, file_name):
    try:
        ...
        filter = annotations_bbox.ImageID.eq(
            file_name) & annotations_bbox.LabelName.eq(class_id)
...
def create_darknet_annotation(df, file_name, class_name_index, output_dir):
    ...
    with open(os.path.join(output_dir, file_name + '.txt'), 'w') as txt_file:
        for arr_values in df[['XMin', 'XMax', 'YMin', 'YMax']].to_numpy():
            ...
            print("{} {} {} {} {}".format(
                class_name_index, center_x, center_y, w, h))
            txt_file.write("{} {} {} {} {}\n".format(
                class_name_index, center_x, center_y, w, h))


# load annotations bounding boxes
train_annotations_bbox = pd.read_csv(
    'OID/csv_folder/train-annotations-bbox.csv')

# load classes csv file
class_descriptions_boxable = pd.read_csv(
    'OID/csv_folder/class-descriptions-boxable.csv')

for (class_name_index, class_name) in enumerate(sorted(os.listdir('OID/Dataset/train'))):
    class_id = get_class_id(class_descriptions_boxable, class_name)
    for img_file in glob('OID/Dataset/train/{}/*.jpg'.format(class_name)):
        file_name = os.path.basename(img_file).split('.')[0]
        file_dir = os.path.dirname(img_file)
        df = get_annotations_dataframe(
            train_annotations_bbox, class_id, file_name)
